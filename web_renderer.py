...
def download_pdf():
    ...
    if not os.path.exists(pdf_path):
        generate_pdf_with_xhtml2pdf(pdf_path)

    return send_file(
        pdf_path, as_attachment=True, download_name="Pawarit_Thareechan_Resume.pdf"
    )
...
def regenerate():
    ...
    try:
        ...
        for file in ["resume.pdf"] + [
            f"static/resume_page_{i}.png" for i in range(1, 5)
        ]:
